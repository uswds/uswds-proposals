...
Our engineering values support our [product values](https://designsystem.digital.gov/about/product-values/) and guide us as we make decisions about USWDS. We originally shared them in our [October 2024 USWDS monthly call](https://designsystem.digital.gov/about/monthly-calls/#october-2024-engineering-values).
...
**Use progressive enhancement.** HTML web components containing relatively simple markup will enable teams to deliver critical content no matter what. Building components this way will ensure that content is available before JavaScript runs (or even if it never does). But this also allows those components to apply all current and future design and behavior customizations that CSS and JavaScript allow if and when they become available.

**Avoid lock-in.** Building with the web platform and using its native idioms prevents USWDS from being tied to any particular framework. Pick tools that aren't the most popular, but most portable. By keeping our reliance on dependencies as low as possible, we’ll be well-positioned to move on from them if they stop working the way we need them to or if a better alternative comes along. 
...
We favor choices that make it easier to work with USWDS.

**User experience shapes the tools.** We need to make sure that we're building tools that help developers build the kinds of sites and services that work for the person using the site or service, wherever they're coming from. 

**Performance matters.** It can be easy to lose track of performance costs when we're using devices powerful enough to make it easy to build sites and services. But performance can have a real usability impact, a cost impact, and an energy impact. We have to work to keep that impact visible. We have to work to keep performance costs low. 
